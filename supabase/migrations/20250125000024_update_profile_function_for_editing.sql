-- Обновляем функцию get_full_profile_data для показа всех изменений на странице редактирования
...
BEGIN
  ...
  IF v_profile IS NOT NULL AND (v_profile->>'primary_profile_id') IS NOT NULL THEN
    ...
    -- Это позволяет показывать изменения сразу после сохранения на странице редактирования
    SELECT json_build_object(
