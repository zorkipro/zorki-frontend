...
import { ProcessedError, getErrorMessage, getFieldErrors } from '@/utils/errorHandler';
...
export const ErrorAlert: React.FC<ErrorAlertProps> = ({
  ...
  if (variant === 'inline') {
    return (
      <div className={`p-3 text-sm ${getTypeStyles()} border rounded-md ${className}`}>
        <div className="flex items-start">
          {getIcon()}
          <div className="ml-2 flex-1">
            {title && <div className="font-medium mb-1">{title}</div>}
            <div>{errorData.message}</div>
            {errorData.isValidationError && Object.keys(errorData.fieldErrors).length > 0 && (
              <div className="mt-2">
                <div className="text-xs font-medium mb-1">Ошибки в полях:</div>
                <ul className="text-xs list-disc list-inside">
                  {Object.entries(errorData.fieldErrors).map(([field, message]) => (
                    <li key={field}>
          ...
          {onDismiss && (
            <Button variant="ghost" size="sm" onClick={onDismiss} className="ml-2 p-1 h-auto">
              <X className="w-4 h-4" />
            </Button>
          )}
        </div>
    ...
    return (
      ...
      >
        <div className="flex items-start p-4">
          <div className={`w-5 h-5 mt-0.5 mr-3 flex-shrink-0 ${getIconColor()}`}>{getIcon()}</div>
      ...
      <AlertDescription>
        ...
        {showDetails && errorData.originalError && (
          <details className="mt-2">
            <summary className="cursor-pointer text-xs">Подробности ошибки</summary>
