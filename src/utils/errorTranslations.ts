...
export const ERROR_TRANSLATIONS: Record<string, string> = {
  ...
  [EXCEPTION_KEYS_ENUM.USERNAME_IS_NOT_TG_CHANNEL]: 'Имя пользователя не является Telegram каналом',
  ...
  [EXCEPTION_KEYS_ENUM.CAN_NOT_LOAD_IG_CLIENT]: 'Не удалось загрузить клиент Instagram',
  ...
  [EXCEPTION_KEYS_ENUM.REQUEST_NOT_BELONG_USER]: 'Запрос не принадлежит пользователю',
  [EXCEPTION_KEYS_ENUM.IG_USER_NOT_FOUND_OR_NO_ACTIVE_PARSER_ACC]:
    'Пользователь Instagram не найден или нет активного парсера',
  [EXCEPTION_KEYS_ENUM.BLOGGER_LINKED_TO_ANOTHER_USER]: 'Блогер привязан к другому пользователю',
  [EXCEPTION_KEYS_ENUM.BLOGGER_NOT_BELONG_USER]: 'Блогер не принадлежит пользователю',
  ...
  [EXCEPTION_KEYS_ENUM.isBarterAvailable]: 'Некорректное значение доступности бартера',
  [EXCEPTION_KEYS_ENUM.isMartRegistry]: 'Некорректное значение реестра маркетплейса',
  ...
  [EXCEPTION_KEYS_ENUM.subCountFrom]: 'Некорректное количество подписчиков (от)',
  ...
  [EXCEPTION_KEYS_ENUM.integrationPriceFrom]: 'Некорректная цена интеграции (от)',
  [EXCEPTION_KEYS_ENUM.integrationPriceTo]: 'Некорректная цена интеграции (до)',
  [EXCEPTION_KEYS_ENUM.coverageSocialType]: 'Некорректный тип социальной сети для охвата',
 ...
 */
export function translateFieldErrors(errorFields: unknown[]): Record<string, string> {
