    ...
    async (id: string) => {
      try {
        ...
        if (error instanceof APIError) {
          ...
          toastRef.current({
            title: 'Ошибка',
            description: error instanceof Error ? error.message : 'Не удалось одобрить блогера',
            variant: 'destructive',
          });
        }
        throw error;
    ...
    async (id: string) => {
      try {
        ...
        if (error instanceof APIError) {
          ...
          toastRef.current({
            title: 'Ошибка',
            description: error instanceof Error ? error.message : 'Не удалось отклонить блогера',
            variant: 'destructive',
          });
        }
        throw error;
