...
export const useProfileForm = (initialData?: EditData): ProfileFormReturn => {
  const initial = useMemo(() => initialData ?? INITIAL_FORM_DATA, [initialData]);
  const [formData, setFormDataState] = useState<EditData>(initial);
  const [activeTab, setActiveTab] = useState<PlatformType | string>(
      ALL_PLATFORMS[0] ?? "instagram",
  );
  const [editingSection, setEditingSection] = useState<string | null>(null);

  /**
   * Частичное обновление данных формы
   */
  ...
  /**
   * Проверка несохранённых изменений (оптимизировано)
   */
  const hasUnsavedChanges = useCallback(
      (originalData: EditData): boolean => {
        return Object.entries(formData).some(([key, currentValue]) => {
          const originalValue = originalData[key as keyof EditData];
          if (Array.isArray(currentValue) && Array.isArray(originalValue)) {
            return (
                currentValue.length !== originalValue.length ||
                currentValue.some((v, i) => v !== originalValue[i])
            );
          }
