...
interface UseStatsFileManagementReturn {
  ...
  confirmDelete: (bloggerId: number, fileId: number, fileName: string) => Promise<void>;
...
export const useStatsFileManagement = (): UseStatsFileManagementReturn => {
  ...
  // Удаление файла
  const deleteFile = useCallback(async (bloggerId: number, fileId: number): Promise<void> => {
    try {
      setDeleting(true);
      setError(null);
